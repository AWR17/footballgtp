...
/**
 * Low-level GET against API-Football, with retry on rate limiting.
 * IMPORTANT: API-Football doesn't always use HTTP 429 for rate limits —
 * it often returns a normal 200 OK with an error message embedded in the
 * response body instead (e.g. { errors: { rateLimit: "Too many
 * requests..." } }). Both shapes are treated as retryable here.
 */
async function apiGet(path, params = {}, { retries = 5, pauseMs = 6500 } = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (res.status === 429) {
      // Rate limited via HTTP status — back off and retry.
    ...
    if (json.errors && Object.keys(json.errors).length > 0) {
      ...
      const isRateLimit = /rate ?limit|too many requests/i.test(errorText);

      if (isRateLimit && attempt < retries) {
        // Rate limited via response body (the common case for this API)
        // rather than HTTP status — back off and retry the same way.
        console.warn(`[api-football] rate limited on ${path}, retrying (attempt ${attempt + 1}/${retries})...`);
        await sleep(pauseMs * (attempt + 1));
        continue;
      }

      throw new Error(`API-Football error for ${path}: ${errorText}`);
    }
