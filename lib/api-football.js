...
async function getTeamsForLeague(leagueId, season) {
  ...
  return resp.map((entry) => ({ id: entry.team.id, name: entry.team.name }));
}

async function getSquad(teamId) {
  const resp = await apiGet("/players/squads", { team: teamId });
  const entry = resp?.[0];
  return (entry?.players || []).map((p) => ({ id: p.id, name: p.name }));
}
...
async function getPlayerProfile(playerId, season) {
  ...
  return {
    ...
    birthDate: entry?.player?.birth?.date ?? null,
  };
}
